...
class HostedService:
    ...
    def __init__(self, store: HostedStore) -> None:
        ...
        statements = (
            """CREATE TABLE IF NOT EXISTS hosted_rooms (
                ...
                bot_paused TEXT, bot_pending INTEGER NOT NULL DEFAULT 0)""",
    ...
    def _member(
        ...
        seat = _row(
            conn,
            "SELECT * FROM hosted_seats WHERE game_id=? AND principal_id=?",
            (game_id, principal),
        )
    ...
    def _validate(controllers: tuple[str, ...], kingdom: tuple[str, ...]) -> None:
        ...
        if any(
            controller != "human" and controller not in STRATEGIES for controller in controllers
        ):
    ...
    def create(
        ...
        kingdom: tuple[str, ...] = GameConfig().kingdom,
    ) -> TableView:
        ...
        with self.store.transaction(write=True) as conn:
            ...
            conn.execute(
                ...
                (game_id, principal, _json(kingdom), str(time.time())),
            )
    ...
    def _view(
        ...
        return TableView(
            game_id,
            principal == _text(room, "host_id"),
            _int(own, "seat"),
            _text(room, "status"),
            _int(room, "lobby_revision"),
            _int(room, "revision"),
            ...
            tuple(
                SeatView(
                    _int(seat, "seat"),
                    _text(seat, "controller"),
                    cast(str, seat["display_name"])
                    if seat["display_name"] is not None
                    else (
                        "Open seat"
                        if seat["controller"] == "human"
                        else f"Bot {_int(seat, 'seat') + 1}"
                    ),
                    seat["principal_id"] is not None or seat["controller"] != "human",
                    seat["ready_revision"] == room["lobby_revision"],
                )
                for seat in seats
            ),
    ...
    def list_tables(self, token: str) -> tuple[TableView, ...]:
        with self.store.transaction() as conn:
            ...
            seats = _rows(
                conn,
                "SELECT * FROM hosted_seats WHERE principal_id=? ORDER BY game_id",
                (principal,),
            )
            return tuple(
                self._view(
                    ...
                    _row(
                        conn,
                        "SELECT * FROM hosted_rooms WHERE game_id=?",
                        (_text(seat, "game_id"),),
                    ),
                    seat,
                )
    ...
    def invite(self, token: str, game_id: str, seat: int) -> Invitation:
        ...
        with self.store.transaction(write=True) as conn:
            ...
            conn.execute(
                "INSERT INTO hosted_invitations VALUES (?,?,?,?,?,0,NULL)",
                (
                    invitation.invitation_id,
                    game_id,
                    seat,
                    _digest(invitation.secret),
    ...
    def join(
        ...
        with nullcontext(conn) if conn is not None else self.store.transaction(write=True) as conn:
            ...
            if invite["claimed_by"] == principal:
                ...
                if (
                    invite["revoked"]
                    or cast(float, invite["expires_at"]) <= self.store.clock()
    ...
    def ready(
        ...
        with self.store.transaction(write=True) as conn:
            ...
            conn.execute(
                "UPDATE hosted_seats SET ready_revision=? WHERE game_id=? AND ready_revision=?",
                (expected_lobby_revision + 1, game_id, expected_lobby_revision),
            )
            conn.execute(
                "UPDATE hosted_seats SET ready_revision=? WHERE game_id=? AND principal_id=?",
    ...
    def configure(
        ...
        with self.store.transaction(write=True) as conn:
            ...
            self._lobby(room, expected_lobby_revision)
            seats = _rows(
                conn, "SELECT * FROM hosted_seats WHERE game_id=? ORDER BY seat", (game_id,)
            )
            for seat in seats:
                index = _int(seat, "seat")
                if seat["principal_id"] is not None and (
                    index >= len(controllers) or controllers[index] != "human"
                ):
            ...
            conn.execute(
                "DELETE FROM hosted_seats WHERE game_id=? AND seat>=?", (game_id, len(controllers))
            )
            for index, controller in enumerate(controllers):
                conn.execute(
                    "INSERT INTO hosted_seats VALUES (?,?,?,NULL,NULL) ON CONFLICT(game_id,seat) DO UPDATE SET controller=excluded.controller",
                    (game_id, index, controller),
                )
            conn.execute(
    ...
    def remove(
        ...
        with self.store.transaction(write=True) as conn:
            ...
            conn.execute(
                "DELETE FROM hosted_preferences WHERE game_id=? AND principal_id=?",
                (game_id, cast(str | None, target["principal_id"])),
            )
            conn.execute(
                "UPDATE hosted_seats SET principal_id=NULL,ready_revision=NULL WHERE game_id=? AND seat=?",
                (game_id, seat),
            )
            conn.execute(
                "UPDATE hosted_invitations SET revoked=1,claimed_by=NULL WHERE game_id=? AND seat=?",
                (game_id, seat),
            )
            self._bump(conn, game_id)
        return self.view(token, game_id)
    ...
    def start(self, token: str, game_id: str, expected_lobby_revision: int) -> TableView:
        with self.store.transaction() as conn:
            ...
            seats = _rows(
                conn,
                "SELECT s.*,p.display_name FROM hosted_seats s LEFT JOIN principals p ON s.principal_id=p.principal_id WHERE game_id=? ORDER BY seat",
                (game_id,),
            )
            if any(
                seat["controller"] == "human"
                and (
                    seat["principal_id"] is None
                    or seat["ready_revision"] != expected_lobby_revision
                )
                for seat in seats
            ):
        ...
        state = new_game(
            GameConfig(
                len(seats), tuple(cast(list[str], json.loads(_text(room, "kingdom")))), names
            ),
            seed,
        )
        bots = tuple(
            BotState(_text(seat, "controller"), seed ^ (_int(seat, "seat") * 0x9E3779B97F4A7C15))
            for seat in seats
            if seat["controller"] != "human"
        )
        ...
        with self.store.transaction(write=True) as conn:
            ...
            conn.execute(
                "UPDATE hosted_rooms SET status='active',snapshot=?,bot_pending=?,lobby_revision=lobby_revision+1 WHERE game_id=?",
                (
                    snapshot,
                    int(bool(state.pending and state.pending.player not in humans)),
                    game_id,
                ),
            )
    ...
    def _commit(
        ...
        game_id = _text(room, "game_id")
        conn.execute(
            "INSERT INTO hosted_commands VALUES (?,?,?,?,?,?)",
            (game_id, revision, actor, request_id, payload, events),
        )
        conn.execute(
            "UPDATE hosted_rooms SET revision=?,snapshot=?,status=?,updated_at=?,bot_pending=? WHERE game_id=?",
            (
                revision,
                snapshot,
    ...
    def set_theme(
        self, token: str, game_id: str, theme_id: str, expected_version: int
    ) -> TableView:
