...
from sway.engine.models import (
    Card, Command, Decision, DecisionKind, Effect, Event, GameConfig, GameState,
    InvalidCommand, OpponentView, Option, Player, PlayerView, Transition,
)
...
def _gain(state: GameState, player: int, card_id: str, destination: str = "discard") -> None:
    ...
    owner = state.players[player]
    zone = owner.hand if destination == "hand" else owner.deck if destination == "deck" else owner.discard
    zone.append(card)
...
def _discard(state: GameState, player: int, cards: list[Card], *, public: bool = False) -> None:
    ...
    if not cards:
        return
    state.events.append(Event("discard", player, tuple(cards), len(cards), None if public else player))
    if not public:
...
def _choose(
    state: GameState, effect: Effect, kind: DecisionKind, prompt: str,
    options: tuple[Option, ...], minimum: int, maximum: int, *, ordered: bool = False,
) -> None:
    if not options:
        return
    state.pending = Decision(
        f"d{state.next_decision_id}", effect.player, kind, prompt, options,
        min(minimum, len(options)), min(maximum, len(options)), ordered,
    )
...
def new_game(config: GameConfig, seed: int) -> GameState:
    ...
    supply = {key: victory_count if "victory" in CATALOG[key].types else 10 for key in config.kingdom}
    supply.update({
        "treasure1": 60 - 7 * config.player_count, "treasure2": 40, "treasure3": 30,
        "victory1": victory_count, "victory2": victory_count, "victory3": victory_count,
        "curse": 10 * (config.player_count - 1),
    })
    names = config.player_names or tuple(f"Player {index + 1}" for index in range(config.player_count))
...
def view_for(state: GameState, player: int) -> PlayerView:
    ...
    return PlayerView(
        ...
        players=tuple(OpponentView(
            owner.name, len(owner.hand), len(owner.deck) if index == player else None, None,
            tuple(owner.discard[-1:]),
            tuple(owner.in_play), tuple(owner.revealed), owner.turns,
        ) for index, owner in enumerate(state.players)),
        hand=tuple(state.players[player].hand), looked=tuple(state.players[player].looked),
        supply=dict(state.supply), trash=tuple(state.trash), active_player=state.active_player,
        phase=state.phase, actions=state.actions, buys=state.buys, coins=state.coins,
        revision=state.revision, turn=state.turn,
        ...
        scores=state.scores, winners=state.winners,
    )
...
def _settle(state: GameState) -> None:
    while state.pending is None and state.phase != "finished":
        if state.effects:
            ...
            available = [card for card in state.players[state.active_player].hand if "action" in CATALOG[card.definition].types]
            if state.actions <= 0 or not available:
                state.phase = "buy"
                continue
            _choose(state, Effect("action", state.active_player), "menu", "action",
                    _options(available) + (Option("end-actions"),), 1, 1)
        else:
            options: list[Option] = []
            if not state.buying_started:
                treasures = [card for card in state.players[state.active_player].hand if "treasure" in CATALOG[card.definition].types]
                options.extend(_options(treasures))
                if treasures:
                    options.append(Option("play-treasures"))
            if state.buys > 0:
                options.extend(Option(card_id, card_id) for card_id, count in state.supply.items()
...
def _play(state: GameState, player: int, card: Card) -> None:
    ...
    if "attack" in CATALOG[card.definition].types:
        ...
        _schedule(state,
            *(Effect("reaction", target, card_id=card.definition, instance_id=card.id) for target in others),
            resolve,
            *(Effect("attack", target, card_id=card.definition, instance_id=card.id) for target in others),
        )
    else:
...
def _finish_turn(state: GameState) -> None:
    ...
    if state.supply["victory3"] == 0 or sum(count == 0 for count in state.supply.values()) >= 3:
        ...
        fewest_turns = min(owner.turns for index, owner in enumerate(state.players) if state.scores[index] == best)
        state.winners = tuple(index for index, owner in enumerate(state.players)
...
def all_cards(player: Player) -> list[Card]:
    return player.deck + player.hand + player.discard + player.in_play + player.revealed + player.looked + player.set_aside


def score_player(player: Player) -> int:
    cards = all_cards(player)
    return sum(len(cards) // 10 if card.definition == "k08" else CATALOG[card.definition].points for card in cards)
...
def _resolve_card(state: GameState, effect: Effect) -> None:
    ...
    draws = {"k06": 4, "k09": 1, "k10": 2, "k12": 1, "k13": 1, "k16": 2, "k18": 1, "k20": 1, "k21": 3, "k24": 1, "k25": 2}
    actions = {"k04": 1, "k07": 2, "k09": 1, "k10": 1, "k12": 1, "k13": 1, "k18": 1, "k20": 1, "k24": 2}
    ...
    if card_id == "k01":
        ...
        _choose(state, Effect("cellar", actor), "select", "cellar", _options(player.hand), 0, len(player.hand), ordered=True)
    elif card_id == "k05":
        ...
        _choose(state, Effect("harbinger", actor), "select", "harbinger", _options(player.discard), 0, 1)
    elif card_id == "k11":
        ...
        _choose(state, Effect("moneylender", actor), "select", "moneylender", _options(choices), 0, 1)
    elif card_id == "k18":
        count = sum(value == 0 for value in state.supply.values())
        if count:
            _choose(state, Effect("discard_hand", actor), "select", "poacher", _options(player.hand), count, count, ordered=True)
    elif card_id == "k19":
        _choose(state, Effect("remodel", actor), "select", "remodel", _options(player.hand), 1, 1)
    elif card_id == "k20":
        player.looked.extend(_take_deck(state, actor, 2))
        _schedule(state, Effect("sentry_trash", actor), Effect("sentry_discard", actor), Effect("sentry_order", actor))
    elif card_id == "k22":
        ...
        if cards and "action" in CATALOG[cards[0].definition].types:
            _choose(state, Effect("vassal", actor, instance_id=cards[0].id), "yes_no", "vassal", (Option("yes"), Option("no")), 1, 1)
    elif card_id == "k26":
...
def _execute(state: GameState, effect: Effect) -> None:
    ...
    if kind == "resolve":
        ...
        options = tuple(Option(key, key) for key, count in state.supply.items()
                        if count > 0 and CATALOG[key].cost <= effect.amount
                        and (effect.target != 1 or "treasure" in CATALOG[key].types))
        _choose(state, effect, "supply", "gain_hand" if effect.card_id == "hand" else "gain", options, 1, 1)
        ...
        _choose(state, effect, "select", kind, _options(player.looked), 0, len(player.looked), ordered=True)
    elif kind == "sentry_order":
        _choose(state, effect, "order", kind, _options(player.looked), len(player.looked), len(player.looked), ordered=True)
    else:
...
def _attack(state: GameState, effect: Effect) -> None:
    ...
    if effect.card_id == "k02":
        ...
        eligible = [card for card in player.revealed if "treasure" in CATALOG[card.definition].types and card.definition != "treasure1"]
        ...
        if eligible:
            _choose(state, Effect("bureaucrat", actor), "select", "bureaucrat", _options(eligible), 1, 1)
        else:
            state.events.append(Event("reveal", actor, tuple(player.hand)))
    elif effect.card_id == "k14":
        count = len(player.hand) - 3
        if count > 0:
            _choose(state, Effect("discard_hand", actor), "select", "militia", _options(player.hand), count, count, ordered=True)
...
def _library(state: GameState, effect: Effect) -> None:
    ...
    while len(player.hand) < 7:
        ...
        if "action" in CATALOG[card.definition].types:
            ...
            _choose(state, Effect("library_choice", actor, instance_id=card.id), "yes_no", "library", (Option("yes", card.definition, card.id), Option("no")), 1, 1)
            return
...
def _answer(state: GameState, effect: Effect, selected: tuple[str, ...]) -> None:
    ...
    if kind == "action":
        ...
        if selected[0] == "yes":
            ...
            state.effects = [item for item in state.effects if not (
                item.kind == "attack" and item.player == actor and item.instance_id == effect.instance_id)]
        ...
        if cards:
            if kind == "moneylender":
                ...
                _schedule(state, Effect("gain", actor, amount, "hand" if kind == "mine" else "", target=1 if kind == "mine" else -1))
    elif kind == "gain":
        ...
        if cards:
            state.events.append(Event("topdeck", actor, tuple(cards), len(cards), actor if kind == "topdeck_hand" else None))
        ...
        if selected:
            card = _remove(player.hand, selected)[0]
            player.in_play.append(card)
            _schedule(state, Effect("play", actor, card_id=card.definition, instance_id=card.id),
